...
@content: 本代码以pycharm为例子,学习使用Python语言
        本程序的目的：
...
class Sequence(object):
    ...
    """
    listEx = [1, 2, 3]  # 列表  # 单行注释'#'如果不顶行，前有两个空格，后有一个空格。
    stringEx = 'this is a string.'  # 字符串
    tupleEx = (['a', 'b'], 2, 3)  # 元组
    def fun_sequence(self):
        """
        一些通用的方法。
        :return:None
        """
        print(stringEx[3])
        print(listEx * 3)  # 重复
        print(listEx + listEx)  # 连接
        print(1 in listEx)  # 判断是否存在该元素
        print('c' not in stringEx)
        fun_slice(stringEx)
        print(cmp(listEx, [1, 2]))
        print(cmp(stringEx, 'this is'))  # 按acsii比较
        print(len(listEx))
        print(max(stringEx)  # min()
        for i, char in enumerate(stringEx):
            print(i, char)
        str1, str2 = 'abcd', 'xyz'
        print(zip(str1, str2))  # 两个字符串中的字符，一一对应
        print(sorted(stringEx, reverse=False))  # 逆序排序，第二个参数可以省略
        print(sum(listEx)  # 注意;只对数值型有效
        for char in reversed(listEx):  # reversed 逆序
            print(char)
        list_oper(listEx)
        string_template('python')
        tuple_no_change(tupleEx)
    ...
    def list_oper(list1):
        ...
        list1.pop()  # 删除最后一个数，
        print(list1.remove(2)  # 删除元素2,如果有多个，只删除第一个
...
if __name__ == '__main__':
    ...
    # 如果本程序是直接执行，__name__='__main__'，如果是被别的程序调用执行，__name__=模块名（PythonExample）
    # 参考http://blog.csdn.net/fengjiexyb/article/details/77838027
    # main()
    # fun_iffor('a')
